...
function downloadExcel(){
    ...
    form.setAttribute('action', "/CUSTOMER/Customer_Category_Management/excel-list")
...
function downloadExcelSample(){
    ...
    form.setAttribute('method', 'post');
    form.setAttribute('action', "/CUSTOMER/Customer_Management/sample-excel-list")
    document.body.appendChild(form);
    form.submit();
    document.body.removeChild(form);
...
function fileSubmit(){
        ...
        var request = $.ajax({
            url:"/CUSTOMER/Customer_Management/uploadlist",
            type:"POST",
            data:formData,
            processData:false,
            contentType: false,
...
//        postData.desc = $('#pop_desc_drop').val();
        ...
        var request = $.ajax({
            url:"/CUSTOMER/Customer_Management/"+ postData.seq,
            type:"PUT",
            data:postData,
            success: function(data, status){
                $('#demo-foo-filtering').DataTable().ajax.reload();
...
function showEditDialog(id){
    ...
    row = $('#demo-foo-filtering').DataTable().data()[rownum];
     $('#pop_seq').val(row.seq);
     $('#pop_Customer_Category').val(row.Customer_Category);
     $("#pop_Customer_Name").val(row.Customer_Name);
     $("#pop_IP_Address").val(row.IP_Address);
     $("#pop_Branch").val(row.Branch);
...
function deleteItem(){
    ...
    if( result) {
        ...
        var request = $.ajax({
            url: "/CUSTOMER/Customer_Management/" + row.seq,
            type: "DELETE",
            success: function (data, status) {
                //alert('success');
                $('#demo-foo-filtering').DataTable().ajax.reload();
...
function GetList(){
    if ($('#demo-foo-filtering').length !== 0) {
       var dtTable = $('#demo-foo-filtering').DataTable({
                ajax: {
                    url:"/CUSTOMER/Customer_Management/list",
                    type:"POST",
                    "data": function (d) {
...
//                }
            ...
            "drawCallback" : function(setting,data){
                ...
                var theTableName = "customerManagement";
                 setTimeout(function(){
