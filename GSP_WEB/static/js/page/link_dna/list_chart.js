var tooltip = d3.select("body")
        ...
        d3.json(url, function (error, json) {
            ...
            link.on("mouseover", function (d) {
                ...
                tooltip.transition()
                    .duration(300)
                    .style("opacity", .9);
                tooltipHtml = "<p>출발 IP {0} 목적 IP {1}</p>".format(d.source.name, d.target.name);
                tooltipHtml += "<p>프로파일 {0} </p>"
                        .format(nullToDesc(d.profile));
                tooltipHtml += "<p>프로토콜 {0} </p>"
                    .format(nullableStringFormat(d.protocol));
                tooltipHtml += "<p>전송량 {0}</p>"
                    .format( getReadableFileSizeString(d.bytes));
                tooltipHtml += "<p>패킷수 {0}</p>"
                    .format( d.pkts.toLocaleString() );
                tooltipHtml += "<p>세션 유지시간 {0} sec</p>"
                    .format( nullableStringFormat(d.s_time));
                tooltipHtml += "<p>세션 수 {0} </p>"
                    .format( nullableStringFormat(d.s_count));
                tooltipHtml += "URG: {0}%, {1} / ACK: {2}%, {3} / PSH: {4}%, {5} / RST: {6}%, {7} / SYN: {8}%, {9} / FIN: {10}%, {11}".format(
                    nullableStringFormat(d.tcp_urg_p)
                    , nullableStringFormat(d.tcp_urg)
                    , nullableStringFormat(d.tcp_ack_p)
                    , nullableStringFormat(d.tcp_ack)
                    , nullableStringFormat(d.tcp_psh_p)
                    , nullableStringFormat(d.tcp_psh)
                    , nullableStringFormat(d.tcp_rst_p)
                    , nullableStringFormat(d.tcp_rst)
                    , nullableStringFormat(d.tcp_syn_p)
                    , nullableStringFormat(d.tcp_syn)
                    , nullableStringFormat(d.tcp_fin_p)
                    , nullableStringFormat(d.tcp_fin)
                );
                tooltipHtml += "<p>지리적 거리 {0} km</p>".format(
                    nullableStringFormat(d.distance)
                );
                ...
                var res = confirm('해당 IP(' + d.source.name + ', '+ d.target.name +')의 GEO 정보를 검색하시겠습니까?');
                datasource = $('#datasource').val();
                if (res)
                    window.location = '/chart/geo-chart?srcip=' + d.target.name + '&dstip=' + d.source.name + '&timestamp=' + d.timestamp
            });
    ...
    function getLineWidth(d)
    {
        opt = $("#lineOpt").val();
        if(opt == 'bytes'){
            return getPercentage(d.bytes, maxdata.bytes);
        }
        if(opt == 'pkts'){
            return getPercentage(d.s_pkts, maxdata.pkts);
        }
        if(opt == 'sessionCnt'){
