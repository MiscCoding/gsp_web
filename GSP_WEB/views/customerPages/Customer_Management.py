...
@blueprint_page.route('/Customer_Management', methods=['GET'])
@login_required
def Customer_List():
...
@blueprint_page.route('/Customer_Management/whitelistPeriodSet', methods=['PUT'])
def setCustomer_DashboardLinkNP():
    whitelist = wl_maintenance_period.query.filter_by(datatype='days').first()
...
@blueprint_page.route('/Customer_Management/list',methods=['POST'] )
def Customer_getlist():
    ...
    query = Integrated_Customer_Management.query

    if keyword != "" and search_keyword_type == "Customer_Category":
        query = query.filter(Integrated_Customer_Management.Customer_Category.like('%'+keyword+'%'))

    if keyword != "" and search_keyword_type == "Customer_Name":
        query = query.filter(Integrated_Customer_Management.Customer_Name.like('%'+keyword+'%'))

    if keyword != "" and search_keyword_type == "IP_Address":
        query = query.filter(Integrated_Customer_Management.IP_Address.like('%'+keyword+'%'))

    if keyword != "" and search_keyword_type == "Branch":
        query = query.filter(Integrated_Customer_Management.Branch.like('%'+keyword+'%'))

    if keyword != "" and search_keyword_type == "Description":
        query = query.filter(Integrated_Customer_Management.Description.like('%' + keyword + '%'))

    # if keyword != "" and search_keyword_type == "type":
    #     query = query.filter(Integrated_Customer_Management.type.like('%' + keyword + '%'))
    ...
    cncList = query.order_by(Integrated_Customer_Management.seq.desc()).paginate(curpage, per_page, error_out=False)

    result = dict()
    result["draw"] = str(draw)
    result["recordsTotal"] = str(cncList.total)
    result["recordsFiltered"] = str(cncList.total)
    result["data"] = Integrated_Customer_Management.serialize_list(cncList.items)
    str_json = json.dumps(result)
    return Response(str_json, mimetype='application/json')

@blueprint_page.route('/ip-customer-list', methods=['POST'])
#@login_required
def add_Customer_Element():
    #exists = Integrated_Customer_Management.query.filter_by(ip=request.form['pattern'].strip()).first()
    # if exists is not None:
    #     raise InvalidUsage('중복 IP가 존재합니다.', status_code=501) # duplication is allowed.

    try:
        _pattern = Integrated_Customer_Management()
        _pattern.Customer_Category = request.form['Customer_Category']
        _pattern.Customer_Name = request.form['Customer_Name']
        _pattern.IP_Address = request.form['IP_Address']
        _pattern.Branch = request.form['Branch']
...
@blueprint_page.route('/Customer_Management/<int:seq>', methods=['PUT'])
#@login_required
def editCustomer_Element_url(seq):
    _pattern = db_session.query(Integrated_Customer_Management).filter_by(seq=seq).first()
    try:
...
@blueprint_page.route('/Customer_Management/<int:seq>', methods=['DELETE'])
#@login_required
def delete_Customer_element_url(seq):
    _pattern = db_session.query(Integrated_Customer_Management).filter_by(seq=seq).first()
...
@blueprint_page.route('/Customer_Management/uploadlist', methods=['POST'])
def addCustomer_ElementFileData():
...
@blueprint_page.route('/Customer_Management/excel-list', methods=['GET','POST'])
#@login_required
def getCustomer_Element_ListExcel_url():
    ...
    # if rowCount > 10000:
    #     rowCount = 10000
    cncList = query.order_by(Integrated_Customer_Management.seq.asc()).paginate(curpage, rowCount, error_out=False)
...
@blueprint_page.route('/Customer_Management/sample-excel-list', methods=['GET','POST'])
#@login_required
def getElement_Customer_sample():
