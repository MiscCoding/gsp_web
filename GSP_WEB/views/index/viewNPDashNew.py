...
from GSP_WEB.models.TI_Dashboard_data import TI_Dashboard_data
...
blueprint_page = Blueprint('bp_NPNewindex_page', __name__, url_prefix='/NewNPindex')
...
def getIndexNPNew():
    ...
    fstBoxfstRow = TI_Dashboard_data.query.filter_by(div_id='fstBoxfstRow').first()
    sndBoxfstRow = TI_Dashboard_data.query.filter_by(div_id='sndBoxfstRow').first()
    trdBoxfstRow = TI_Dashboard_data.query.filter_by(div_id='trdBoxfstRow').first()
    fothBoxfstRow = TI_Dashboard_data.query.filter_by(div_id='fothBoxfstRow').first()
    fstBoxsndRow = TI_Dashboard_data.query.filter_by(div_id='fstBoxsndRow').first()
    sndBoxsndRow = TI_Dashboard_data.query.filter_by(div_id='sndBoxsndRow').first()
    trdBoxsndRow = TI_Dashboard_data.query.filter_by(div_id='trdBoxsndRow').first()
    fothBoxsndRow = TI_Dashboard_data.query.filter_by(div_id='fothBoxsndRow').first()
    fstBoxtrdRow = TI_Dashboard_data.query.filter_by(div_id='fstBoxtrdRow').first()
    sndBoxtrdRow = TI_Dashboard_data.query.filter_by(div_id='sndBoxtrdRow').first()
    trdBoxtrdRow = TI_Dashboard_data.query.filter_by(div_id='trdBoxtrdRow').first()
    fothBoxtrdRow = TI_Dashboard_data.query.filter_by(div_id='fothBoxtrdRow').first()
    return render_template('index/dashboard_kibanaNPNew.html', kibana_link = uri.EXT1, fstBoxfstRowTitle=fstBoxfstRow.title, fstBoxfstRowURL= fstBoxfstRow.url, \
                           sndBoxfstRowTitle=sndBoxfstRow.title, sndBoxfstRowURL=sndBoxfstRow.url, trdBoxfstRowTitle= trdBoxfstRow.title, trdBoxfstRowURL=trdBoxfstRow.url, \
                           fothBoxfstRowTitle = fothBoxfstRow.title, fothBoxfstRowURL = fothBoxfstRow.url, fstBoxsndRowTitle=fstBoxsndRow.title, fstBoxsndRowURL=fstBoxsndRow.url, \
                           sndBoxsndRowTitle= sndBoxsndRow.title, sndBoxsndRowURL=sndBoxsndRow.url, trdBoxsndRowTitle=trdBoxsndRow.title, trdBoxsndRowURL=trdBoxsndRow.url, \
                           fothBoxsndRowTitle=fothBoxsndRow.title, fothBoxsndRowURL= fothBoxsndRow.url, fstBoxtrdRowTitle=fstBoxtrdRow.title, fstBoxtrdRowURL = fstBoxtrdRow.url, \
                           sndBoxtrdRowTitle = sndBoxtrdRow.title, sndBoxtrdRowURL=sndBoxtrdRow.url, trdBoxtrdRowTitle = trdBoxtrdRow.title, trdBoxtrdRowURL = trdBoxtrdRow.url, \
                           fothBoxtrdRowTitle = fothBoxtrdRow.title, fothBoxtrdRowURL = fothBoxtrdRow.url)

@blueprint_page.route('/DashboardLinkNPNew', methods=['POST'])
...
def todayUrlAnalysisNPNew(request, query_type = "uri"):
    per_page = 1
    start_idx = 0
    end_dt = "now/d"
    str_dt = "now-1d/d"
...
def todayFileAnalysisNPNew(request, query_type = "file"):
    per_page = 1
    start_idx = 0
    end_dt = "now/d"
    str_dt = "now-1d/d"
...
def totalMaliciousUrlQueryNPNew(request, query_type = "uri"):
    per_page = 1
    start_idx = 0
    end_dt = "now/d"
    str_dt = "now-1d/d"
...
def totalMaliciousQueryNPNew(request, query_type):
    per_page = 1
    start_idx = 0
    end_dt = "now/d"
    str_dt = "now-1d/d"
...
def todayURLFileCountNPNew(type, device):
    end_dt = "now/d"
    str_dt = "now-1d/d"
    query = {
...
@blueprint_page.route('/getTopBoardNPNew')
def getTopBoardNPNew():
    ...
    MFdoc = todayURLFileCountNPNew("file", "NPC")
...
@blueprint_page.route('/getLineChartNPNew')
def getLineChartDataNPNew():
...
@blueprint_page.route('/getBarChartNPNew')
def getBarChartDataNPNew():
...
@blueprint_page.route('/getGridNPNew')
def getGridNPNew():
...
@blueprint_page.route('/getWorldChartNPNew')
def getWorldChartNPNew():
    es = Elasticsearch([{'host': app.config['ELASTICSEARCH_URI'], 'port': int(app.config['ELASTICSEARCH_PORT'])}])
