...
blueprint_page = Blueprint('bp_NPindex_page', __name__, url_prefix='/NPindex')
...
def getIndexNP():
    ...
    return render_template('index/dashboard_kibanaNP.html', kibana_link = uri.EXT1)

@blueprint_page.route('/DashboardLinkNP', methods=['PUT'])
...
def todayUrlAnalysisNP(request, query_type = "uri"):
    per_page = 1
    start_idx = 0
    end_dt = "now/d"
    str_dt = "now-1d/d"
...
def todayFileAnalysisNP(request, query_type = "file"):
    per_page = 1
    start_idx = 0
    end_dt = "now/d"
    str_dt = "now-1d/d"
...
def totalMaliciousUrlQueryNP(request, query_type = "uri"):
    per_page = 1
    start_idx = 0
    end_dt = "now/d"
    str_dt = "now-1d/d"
...
def totalMaliciousQueryNP(request, query_type):
    per_page = 1
    start_idx = 0
    end_dt = "now/d"
    str_dt = "now-1d/d"
...
def todayURLFileCountNP(type, device):
    end_dt = "now/d"
    str_dt = "now-1d/d"
    query = {
...
@blueprint_page.route('/getTopBoardNP')
def getTopBoardNP():
    ...
    MFdoc = todayURLFileCountNP("file", "NPC")
...
@blueprint_page.route('/getLineChartNP')
def getLineChartDataNP():
...
@blueprint_page.route('/getBarChartNP')
def getBarChartDataNP():
...
@blueprint_page.route('/getGridNP')
def getGridNP():
...
@blueprint_page.route('/getWorldChartNP')
def getWorldChartNP():
    es = Elasticsearch([{'host': app.config['ELASTICSEARCH_URI'], 'port': int(app.config['ELASTICSEARCH_PORT'])}])
