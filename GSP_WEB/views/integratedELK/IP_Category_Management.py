...
@blueprint_page.route('/IP_Category', methods=['GET'])
@login_required
def ip_category_List():
    logUtil.addLog(request.remote_addr,1,'ELK > IP_Category_Management')
    whitelist = wl_maintenance_period.query.filter_by(datatype='days').first()
    return render_template('integratedELK/IP_Category_Management.html'
                           # ,
...
@blueprint_page.route('/IP_Category/whitelistPeriodSet', methods=['PUT'])
def setIP_Category_DashboardLinkNP():
    whitelist = wl_maintenance_period.query.filter_by(datatype='days').first()
...
@blueprint_page.route('/IP_Category/list',methods=['POST'] )
def IP_Category_getlist():
    ...
    query = Integrated_IP_Category.query

    if keyword != "" and search_keyword_type == "Field_Name":
        query = query.filter(Integrated_IP_Category.Field_Name.like('%'+keyword+'%'))

    if keyword != "" and search_keyword_type == "Description":
        query = query.filter(Integrated_IP_Category.Description.like('%' + keyword + '%'))

    # if keyword != "" and search_keyword_type == "type":
    #     query = query.filter(Integrated_IP_Category.type.like('%' + keyword + '%'))
    ...
    cncList = query.order_by(Integrated_IP_Category.seq.desc()).paginate(curpage, per_page, error_out=False)

    result = dict()
    result["draw"] = str(draw)
    result["recordsTotal"] = str(cncList.total)
    result["recordsFiltered"] = str(cncList.total)
    result["data"] = Integrated_IP_Category.serialize_list(cncList.items)
    str_json = json.dumps(result)
    return Response(str_json, mimetype='application/json')

@blueprint_page.route('/ip-category-list', methods=['POST'])
#@login_required
def add_Category_IP_url():
    #exists = Integrated_IP_Category.query.filter_by(ip=request.form['pattern'].strip()).first()
    # if exists is not None:
    #     raise InvalidUsage('중복 IP가 존재합니다.', status_code=501) # duplication is allowed.

    try:
        _pattern = Integrated_IP_Category()
...
@blueprint_page.route('/IP_Category/<int:seq>', methods=['PUT'])
#@login_required
def editIP_Category_url(seq):
    _pattern = db_session.query(Integrated_IP_Category).filter_by(seq=seq).first()
    try:
...
@blueprint_page.route('/IP_Category/<int:seq>', methods=['DELETE'])
#@login_required
def delete_ip_category_url(seq):
    _pattern = db_session.query(Integrated_IP_Category).filter_by(seq=seq).first()
...
@blueprint_page.route('/IP_Category/uploadlist', methods=['POST'])
def addIP_CategoryFileData():
...
@blueprint_page.route('/IP_Category/excel-list', methods=['GET','POST'])
#@login_required
def getIP_Category_ListExcel_url():
    ...
    #     query = query.filter(Integrated_IP_Category.ip.like('%'+keyword+'%'))
    #
    # if keyword != "" and search_keyword_type == "url":
    #     query = query.filter(Integrated_IP_Category.url.like('%' + keyword + '%'))
    if keyword != "" and search_keyword_type == "Field_Name":
        query = query.filter(Integrated_IP_Category.Field_Name.like('%'+keyword+'%'))

    # if keyword != "" and search_keyword_type == "IP_Address":
    #     query = query.filter(Integrated_IP_Category.IP_Address.like('%' + keyword + '%'))
    ...
    # if rowCount > 10000:
    #     rowCount = 10000
    cncList = query.order_by(Integrated_IP_Category.seq.asc()).paginate(curpage, rowCount, error_out=False)
...
@blueprint_page.route('/IP_Category/sample-excel-list', methods=['GET','POST'])
#@login_required
def getCategory_IP_sample():
